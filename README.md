...
以下预览由功能录屏生成（GIF，GitHub 可内联播放）。每段下方标注了演示内容，并给出原始高清 MP4 的链接（存放于 `docs/videos/`）。

### 账号登录与角色

![账号登录：管理员 admin 与投稿者 contributor 的登录入口](docs/preview/login.gif)

**账号登录**：访问 `/login` 登录，会话由服务端下发 Cookie 并做 CSRF 校验，**首次登录强制修改初始密码**。平台内置两类角色：

| 账号 | 角色 | 权限 |
| --- | --- | --- |
| `admin` | 管理员 | 管理投稿者 `contributor` 收集并上传的甲秀楼相关作品与影像：审核、发布或退回投稿；管理用户账号与角色；查看审计记录 |
| `contributor` | 协作者 | 提交题咏资料或补充扫描影像，查看并追踪自己投稿的审核进度 |

投稿先进入待审核队列，只有管理员发布后才会进入公开展示。

> [查看高清录屏（MP4）](docs/videos/login/login.mp4)

### 主页与四季场景

![主页：实景高斯泼溅舞台、季候题咏面板与浮玉客唤出入口](docs/preview/home.gif)

**主页**：甲秀楼实景高斯泼溅舞台与「浮玉四时」题名；左下方为季候题咏面板，随当前季节检索并推送经审核的题咏与证据引文；右侧导轨可切换四季、调节画质、开关环境音；右下角可唤出浮玉客。

> [查看高清录屏（MP4）](docs/videos/home/home.mp4)

### 2D / 3D 观看方式切换

![2D/3D 切换：实景三维与轻量二维影像之间来回切换](docs/preview/mode-2d-3d.gif)

**观看方式切换**：在实景三维高斯泼溅（约 17.8 MB 采集资产）与轻量二维影像之间来回切换。浏览器未提供稳定 WebGL2、内存紧张或已开启省流量模式时，会自动降级为二维影像，四时与题咏控制仍然可用。

> [查看高清录屏（MP4）](docs/videos/mode-2d-3d/mode-2d-3d.mp4)

### 四季场景

![春季场景：春雨氛围](docs/preview/spring.gif)

**春雨**：切换至春景后，舞台叠加雨丝与湿润雾气，主色与季候题咏面板同步更新，并推送经审核的春季题咏。

> [查看高清录屏（MP4）](docs/videos/spring/spring.mp4)

![夏季场景：夏日氛围](docs/preview/summer.gif)

**夏日**：水光与蝉噪氛围层叠加，环境音随季节切换；季候题咏面板改推夏季关联作品。

> [查看高清录屏（MP4）](docs/videos/summer/summer.mp4)

![秋季场景：秋叶氛围](docs/preview/autumn.gif)

**秋叶**：落叶粒子与暖色调渲染，季候题咏面板给出秋季题咏及其证据引文。

> [查看高清录屏（MP4）](docs/videos/autumn/autumn.mp4)

![冬季场景：冬雪氛围](docs/preview/winter.gif)

**冬雪**：落雪与冷调渲染；系统检测到「减少动态效果」偏好时会自动降低粒子强度，保证可读性。

> [查看高清录屏（MP4）](docs/videos/winter/winter.mp4)

### 题咏志（题咏检索与筛选）

![题咏志：全文检索、研究范围与季节筛选、相关度与年代排序](docs/preview/archive.gif)

**题咏志**：可按题名、正文、作者全文检索，并按研究范围（直接题咏甲秀楼 / 旧址营建文本 / 同址前史 / 毗邻景观 / 全部研究范围）、季节、版式、真实性、完整度与有无影像组合筛选，支持相关度与年代排序；命中位置会标注为「题名命中 / 正文命中 / 著录命中」。

> [查看高清录屏（MP4）](docs/videos/archive/archive.mp4)

### 题咏扫描影像（原迹与释读对读）

![题咏扫描影像：古籍扫描与释读文本并置查看](docs/preview/facsimile.gif)

**影像对读**：作品详情右侧「影像记录」打开档案阅读台，左为古籍扫描、右为校录正文，可翻页（缩略页 / 前页 / 后页）、缩放（适窗 / 原大 / 自定义）、旋转、全屏与拖移查看，并可下载 JP2 母版；投稿者亦可在此补充上传扫描件，上传后进入管理员审核队列。

> [查看高清录屏（MP4）](docs/videos/facsimile/facsimile.mp4)

### 浮玉客（题咏导览问答）

![浮玉客：随行导览问答，答案附引文](docs/preview/guide.gif)

**浮玉客**：随行导览问答。提问后结合当前检索到的题咏证据作答，答案附引文；未配置模型密钥时退化为确定性导览，无检索证据时会明确说明这是对谈交流而非文献事实。

> [查看高清录屏（MP4）](docs/videos/guide/guide.mp4)

### 诗内·浮玉客（就当前篇目追问）

![诗内浮玉客：在作品详情页直接就当前篇目追问](docs/preview/guide-in-poem.gif)

**诗内浮玉客**：打开某篇作品详情时，浮玉客会自动获知当前篇目，可直接就这一首诗追问，无需再说明是哪一篇。

> [查看高清录屏（MP4）](docs/videos/guide-in-poem/guide-in-poem.mp4)
...
├─ docs/                       项目文档
│   ├─ preview/                界面录屏的 GIF 预览（README 内联播放）
│   └─ videos/                 功能录屏原始 MP4（按功能分目录）
